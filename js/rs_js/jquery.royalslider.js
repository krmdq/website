...
(function($) {
	...
	function RoyalSlider(element, options) {
		...
		var addCursors = function() {
			if(self.st.sliderDrag) {
				...
				if (br.msie || br.opera) {
					...
					self._grabCursor = "default";
					self._grabbingCursor = "default";
		...
		//auto hiding control navigation
/*
		if(self.st.controlNavigation) {
			var container = self._controlsContainer, hoverEl = container, controlNav = self._controlNav;
			hoverEl.hover(
					function() {
						controlNav.removeClass('rsHidden');
					},
					function() {
						controlNav.addClass('rsHidden');
					}
				);
...
*/
		...
		if(self.st.controlNavigation) {
			var controlNav = self._controlNav, $dirNav = $("#dir_nav");
			if ($dirNav.length)
				controlNav.css( 'margin-top' , $dirNav.height() );
		}
		...
		self.ev.on('rsOnUpdateNav', function () {
			if (self._playAll) {
				...
				var slideTitle = currItem.find('.rsTmb_title').text();
				var slideBrand = currItem.find('.rsTmb_brand').text();
				var slideTitleHtml = '<div class="slideTitle">' + slideTitle + '</div>' + '<div class="slideBrand">' + slideBrand + '</div>';

				$dirName.append(slideTitleHtml);

				if ($dirMenu) {
					$dirMenu.addClass('dirNone');
				}

				if ($rsNav) {
					$rsNav.addClass('dirNone');
				}
		...
		//XXX sefari 5.1.9 hover hack
		var chrome_name = /Chrome/gi;
		var safari_name = /Safari/gi;
		var safari_version = /6.[0-9]+.[0-9]+ Safari/gi;
		
		if (!chrome_name.test(navigator.appVersion)) {
			if (safari_name.test(navigator.appVersion)) {
				if (!safari_version.test(navigator.appVersion)) {
					$('.rsTmb_info').each( function() {
						$(this).addClass("dirNavShow");
					});
				}
			}
		}
